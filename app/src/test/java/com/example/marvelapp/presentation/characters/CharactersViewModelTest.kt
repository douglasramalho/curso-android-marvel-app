...
import com.app.core.usecase.GetCharactersUseCase
...
class CharactersViewModelTest {
    ...
    private val pagingDateCharacters = PagingData.from(
        listOf(
    ...
    @ExperimentalCoroutinesApi
    @Test
    fun `should validate the paging data object calling when calling charactersPagingData`() =
        runBlockingTest {
            whenever(
                getCharactersUseCase.invoke(any())
            ).thenReturn(
                flowOf(
                    pagingDateCharacters
                )
            )
